...
router.post("/bookroom", async (req, res) => {
  ...
  try {
    // Check if there are any existing bookings for the same room within the given date range
    ...
    if (existingBooking) {
      return res.status(400).json({ message: "This room is not available during the selected dates" });
    }
